...
currency_price=[]
for i in row:
    currency_name.append(i.find_elements(By.TAG_NAME , 'td')[0].find_element(By.TAG_NAME , 'div').find_element(By.CLASS_NAME , 'highlight').text)
    #x=float(i.find_elements(By.TAG_NAME , 'td')[2].find_element(By.TAG_NAME , 'div').text)
    currency_price.append(i.find_elements(By.TAG_NAME , 'td')[2].find_element(By.TAG_NAME , 'div').text)
...
def exchange_calculate():
    result=0
    if(len(first_price_entry.get())==0 and len(second_price_entry.get())==0):
        messagebox.showinfo(title="ERROR",message="Please enter all data")
    elif(len(first_price_entry.get())==0 and len(second_price_entry.get())!=0):
        if(second_price_entry.get().isdigit()):
            a = currency[currency1.get()]
            b = currency[currency2.get()]
            result=int(second_price_entry.get())*float(b)/float(a)
            c=str(result)
            first_price_entry.insert(0,c)
    elif(len(first_price_entry.get())!=0 and len(second_price_entry.get())==0):
        if(first_price_entry.get().isdigit()):
            x=currency[currency1.get()]
            y=currency[currency2.get()]
            result =int(first_price_entry.get())*float(x)/float(y)
            z=str(result)
            #print(result)
            second_price_entry.insert(0,z)
    elif(len(first_price_entry.get())!=0 and len(second_price_entry.get())!=0):
        messagebox.showinfo(title="ERROR",message="Please leave one entrybox empty")

calculate_btn = Button(text="Calculate",command=exchange_calculate)
calculate_btn.config(pady=7)
calculate_btn.place(x=160,y=420)
